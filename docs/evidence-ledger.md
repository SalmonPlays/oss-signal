...
| Installable CLI | https://www.npmjs.com/package/oss-signal | `0.9.1` is `latest` | Reviewers can run `npm exec --yes --package=oss-signal@0.9.1 -- oss-signal --version` from a clean temporary directory. |
...
| GitHub Action release | https://github.com/SalmonPlays/oss-signal/tree/v0.9.1 | Published tag | Public Action tag used by repository workflows; the separate public demo remains v0.8.4 evidence until refreshed. |
...
| Main repository dogfood | https://github.com/SalmonPlays/oss-signal/actions/workflows/repository-health.yml | Passing | Runs `SalmonPlays/oss-signal@v0.9.1` against this repository. |
