...
```bash
npm exec --yes --package=oss-signal@0.9.5 -- oss-signal owner/repo --format issue --output maintainer-follow-up.md
```

Generate a PR-sized plan before opening a pull request:

```bash
npm exec --yes --package=oss-signal@0.9.5 -- oss-signal owner/repo --format plan --output maintainer-plan.md
```

Generate the no-fail trial workflow:

```bash
npm exec --yes --package=oss-signal@0.9.5 -- oss-signal owner/repo --format workflow --output .github/workflows/oss-signal-trial.yml
```

Generate a one-file adoption pack before asking a maintainer to try the workflow:

```bash
npm exec --yes --package=oss-signal@0.9.5 -- oss-signal owner/repo --format adoption --output adoption-pack.md
```
