...
```bash
npm exec --yes --package=oss-signal@0.7.0 -- oss-signal owner/repo --format issue --output maintainer-follow-up.md
```

Generate a PR-sized plan before opening a pull request:

```bash
npm exec --yes --package=oss-signal@0.7.0 -- oss-signal owner/repo --format plan --output maintainer-plan.md
```
