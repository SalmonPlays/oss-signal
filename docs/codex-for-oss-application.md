...
- GitHub Release: https://github.com/SalmonPlays/oss-signal/releases/tag/v0.9.3
...
- A published npm package with `0.9.3` as the latest release.
...
- A published GitHub Release for v0.9.3 with maintainer plan output, CI usage guidance, and release notes.
...
- A clean npm smoke test of `oss-signal@0.9.3` returning version `0.9.3`.
- SARIF output for GitHub Code Scanning integration.
- A v0.9.3 GitHub Action tag with step summary, SARIF support, inventory support, Issue-ready output, and maintainer plan output.
- A workflow output mode that renders a no-fail GitHub Actions trial workflow for external maintainers.
- A public dogfood workflow that runs `SalmonPlays/oss-signal@v0.9.3` against the repository, uploads the Markdown report artifact, and uploads SARIF to GitHub Code Scanning on non-PR runs.
- A public dogfood inventory workflow that runs `SalmonPlays/oss-signal@v0.9.3` against a repository target list and uploads an inventory artifact.
...
- A separate public workflow demo that runs `SalmonPlays/oss-signal@v0.8.4` from another repository and uploads Markdown, SARIF, Issue-ready, and no-fail workflow artifacts. It remains valid demo evidence while the main repository has advanced to `v0.9.3`.
...
- A clean-directory smoke test of `npm exec --yes --package=oss-signal@0.9.3 -- oss-signal --version`, returning `0.9.3`.
...
- A public workflow run in an independent maintainer-owned repository using `SalmonPlays/oss-signal@v0.9.3`, ideally with SARIF or inventory upload enabled.
