...
| Clean package run | A clean npm execution from a temporary directory should print `0.9.8`. | `npm exec --yes --package=oss-signal@0.9.8 -- oss-signal --version` |
...
`oss-signal` is still early and should not be described as widely adopted. The strongest evidence today is that it is a real, installable maintainer tool with a published npm package, GitHub Action, Marketplace listing, release automation, CI/security workflows, a no-fail external trial path, public field-audit follow-up, one outside-maintainer-accepted PR, and one inbound external contributor PR from a public external fork.
