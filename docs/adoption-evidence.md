...
The npm downloads API returned 3589 downloads for the last-month window from 2026-05-15 to 2026-06-13 when checked on 2026-06-14. Download counts can lag publication, so this is treated as supporting evidence rather than proof of broad adoption.
...
Local self-audit returned score `100`, grade `A`. Public GitHub URL report generation completed during this verification pass, and repository workflows use the public `v0.9.8` Action tag with `GITHUB_TOKEN`.
...
- GitHub Marketplace listing: published, with `v0.9.8` available as the current Action tag after release
...
The npm registry previously returned `0.8.4` for both the package version and `latest` dist-tag on 2026-06-05T16:02:53Z. The 2026-06-12 release updates the expected latest version to `0.9.8`. The 2026-06-14 download check returned 3589 downloads for the last-month window.
...
The four currently visible field-audit follow-up PRs were still open when checked from GitHub on 2026-06-14T05:24:03.072Z. The Divyesh issue was posted on 2026-06-05T04:18:46Z and remained open at that check. Open PRs and issues are not claimed as accepted adoption unless a maintainer merges, replies, or otherwise endorses them.
...
The current repository self-audit score is 100/100, the GitHub community profile health score is 100, and CI verifies the local GitHub Action wrapper. The public `v0.9.8` Action tag is used by the repository health workflow for Markdown, adoption-pack, checksum-manifest, and SARIF output after release. The published npm `0.9.8` package should return version `0.9.8` from a clean temporary directory.
