...
The npm package is publicly available as `oss-signal@0.9.6` with `latest` pointing at `0.9.6` after the release workflow completes.
...
Local self-audit returned score `100`, grade `A`. Public GitHub URL report generation completed during this verification pass, and repository workflows use the public `v0.9.6` Action tag with `GITHUB_TOKEN`.
...
- GitHub Marketplace listing: published, with `v0.9.6` available as the current Action tag after release
...
The npm registry previously returned `0.8.4` for both the package version and `latest` dist-tag on 2026-06-05T16:02:53Z. The 2026-06-07 release updates the expected latest version to `0.9.6`. The 2026-06-08 download check returned 356 downloads for the last-month window.
